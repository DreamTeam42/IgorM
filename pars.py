...
def parse(html):
    ...
    e9 = soup1.find('auctionstepminpercent')
    if e9 is None:
        e9 = None
    else:
      e9 = soup1.find('auctionstepminpercent').text
    e10 = soup1.find('auctionstepmaxpercent')
    if e10 is None:
        e10 = None
    else:
...
def main():
    def get_html2(base_url):
        return requests.get(base_url).text

    #Кол-во ссылок на одной странице
    CountOfPage = 20
    #Основной адрес сайта
    main_url = get_html2('http://utp.sberbank-ast.ru/VIP/List/PurchaseList')
    soup2 = BeautifulSoup(main_url,'html.parser')
    #Находим значение value
    value = soup2.find('div',id='wrapper').find('div',class_='master_open_content').find('div',class_='container').find('div',id='dataPanel').find('input',id='xmlData').get('value')
    #Преобразуем значение value в объект "Супа"
    soup3 = BeautifulSoup(value,'html.parser') 
    er = soup3.find('list').find_all('row')
    #Массивы, хранящие в себе значения индивидуальных частей ссылок
    val1_mass = []
    val2_mass = []
    index2 = 0
    index3 = 0
    #Максимальное кол-во значений индивидуальных частей в каждом массиве
    full = 20
    # Цикл для нахождения и занесения найденных значений в массивы
    for n in er:
        i = n.find('typeid').text
        h = n.find('id').text
        val1_mass.insert(index2,i)
        val2_mass.insert(index2,h)
        if index2<full:
          index2+=1
    # Цикл парсинга ссылок для одной страницы 
    while index3<CountOfPage:
        #Cобираем ссылку для парсинга
        link = 'http://utp.sberbank-ast.ru/VIP/NBT/PurchaseView/'+val1_mass[index3]+'/0/0/'+val2_mass[index3]
        #Парсим саму ссылку
        parse(get_html1(link))
        #После парсинга ссылки ждём 2 секунды, чтобы не забанили
        sleep(2)
        index3+=1
